...
abund_data_use = abund_data

move_to_dir('Results_limited')
...
# fa.fit(abund_data_norm)
# visualize_components(fa.components_, use_cols, prefix='fa')
# add_curve(calculate_explained_variance(abund_data_norm, fa.transform(abund_data_norm)), label='Factor', axes=ax)
#
...
kernel_pca = KernelPCA(n_components=len(use_cols), n_jobs=4, copy_X=True, kernel='poly')
kernel_pca.fit(abund_data_use)
add_curve(calculate_explained_variance(abund_data_use, kernel_pca.transform(abund_data_use)), label='KernelPCA', axes=ax)
